...
Lemma primitive_value_store : forall sr fr m gmp addr n w p,
    INV sr fr ->
    sglob_val sr (f_inst fr) global_mem_ptr = Some (VAL_num (VAL_int32 (N_to_i32 gmp))) ->
    smem sr (f_inst fr) = Some m ->
    (Z.of_N gmp + Z.of_N page_size < Z.of_N (mem_length m))%Z ->
    (8 <= n)%N ->    
    (* (-1 < Z.of_N (gmp + n) < Wasm_int.Int32.modulus)%Z -> *)
    (addr+n <= gmp)%N ->
    (exists n', addr = 2 * n')%N ->    
    wasm_i64_prim_related w p ->    
    exists sr',
      INV sr' fr /\
      repr_val_LambdaANF_Wasm (Vprim p) sr' (f_inst fr) (Val_ptr addr).
Proof.
  intros sr fr m gmp addr n w p Hinv Hgmp Hmem HenoughM Hn Haddr Haddr' Hwp.
  have I := Hinv. destruct I as [_ [_ [_ [_ [_ [_ [Hlinmem _]]]]]]].
  destruct Hlinmem as [Hmem1 [m' [Hmem2 [size [<- [Hmem4 Hmem5]]]]]].
  assert (m = m') by congruence. subst m'.
  assert (HgmpBounded: (Z.of_N gmp < Wasm_int.Int32.modulus)%Z). {
    apply mem_length_upper_bound in Hmem5. cbn in Hmem5. simpl_modulus. cbn. lia. }
  assert (Hstore: forall w',
           exists mem, store m (Wasm_int.N_of_uint i32m (N_to_i32 addr)) 0%N
                         (bits (VAL_int64 w'))
                         8 = Some mem). {
    intros.
    apply enough_space_to_store. cbn.
    ...
    rewrite Hl. clear Hl. cbn.
    rewrite Wasm_int.Int32.Z_mod_modulus_id; try lia. }
  destruct (Hstore w) as [m' Hm'].
  remember (upd_s_mem sr (set_nth m' sr.(s_mems) 0 m')) as sr'.
  exists sr'.
  assert (Hstore': smem_store sr (f_inst fr) (Wasm_int.N_of_uint i32m (N_to_i32 addr)) 0%N
                  (VAL_int64 w) T_i64 = Some sr'). {
    unfold smem_store. rewrite Hmem1. cbn. subst sr'.
    unfold smem in Hmem2. rewrite Hmem1 in Hmem2. destruct (s_mems sr)=>//.
  ...
  assert (Hinv' : INV sr' fr). {
    ...
    congruence. exists (mem_size m); split; auto. unfold mem_size. congruence. now subst sr'. }
  assert (Hgmp': sglob_val sr' (f_inst fr) global_mem_ptr = Some (VAL_num (VAL_int32 (N_to_i32 gmp)))) by now subst sr'.
  (* have I := Hinv'. destruct I as [_ [_ [_ [Hgmp_w [_ [_ [Hlinmem' [_ [_ [_ [_ [_ [_ [Hgmp_mult_two]]]]]]]]]]]]]].   *)
  split. assumption.
  apply Rprim_v with (w:=w) (gmp:=gmp%N) (m:=m').
  assumption.
  lia.
  lia.
  assumption.
  unfold smem_store, smem in *.
  destruct (lookup_N (inst_mems (f_inst fr)) 0) eqn:Hl1. 2: inv Hmem.
  destruct (lookup_N (s_mems sr)  m0) eqn:Hl2.
  cbn in Hstore'.
  replace m1 with m in * by congruence.
  rewrite Hm' in Hstore'.
  subst sr'.
  unfold upd_s_mem in *.
  cbn in *.
  inv Hstore'.
  unfold lookup_N.
  erewrite set_nth_nth_error_same. reflexivity. eassumption. now subst sr'.
  assumption.
  assert ((wasm_deserialise (bits (VAL_int64 w)) T_i64) = (VAL_int64 w)) by now apply deserialise_bits.
  rewrite -H.
  apply (store_load_i64 m m' addr (bits (VAL_int64 w))); auto.
  (* cbn in Hm'. *)
  assert (Wasm_int.N_of_uint i32m (N_to_i32 addr) = addr). {
    cbn. rewrite Wasm_int.Int32.Z_mod_modulus_id; lia. }
  rewrite -H0.
  apply Hm'.
Qed. 
...
Proof.
  ...
  { (* Binary operations *)
    ...
    assert (HloadStep: forall es,
      ...
      apply r_eliml; auto.
      elimr_nary_instr 1.
      eapply r_load_success; try eassumption. rewrite -H6. apply Hload2'.
    ...
    assert (addr2 = (Wasm_int.N_of_uint i32m (wasm_value_to_i32 (Val_ptr addr2)))). {
      cbn. rewrite Wasm_int.Int32.Z_mod_modulus_id. now rewrite N2Z.id.
      inv Hval_y; try discriminate. now inv H16. }
    ...
    assert (Hmod : forall n0,
               (Z.land n0 9223372036854775807 = n0 mod Wasm_int.Int64.half_modulus)%Z).
    {
      intros.
      rewrite -H8.
    ...
    { (* eqb *)
      ...
      destruct (Wasm_int.Int64.eq_dec w1 w2) eqn:Heq. {
        ...
        assert ((i1 =? i2)%uint63 = true). {
          ...
          apply inj_pair2 in H14. subst i2. now apply Uint63.eqb_spec. }
        rewrite H14 in HprimResSome.
        assert (v = Vconstr t_true []) by congruence.
        assert (get_ctor_ord cenv t_true = Ret 0%N). { unfold get_ctor_ord. now rewrite H16. }
        assert (get_ctor_arity cenv t_true = Ret 0). { unfold get_ctor_arity. now rewrite H16. }
        destruct (H4 _ _ H18 H20 H19) as [fr [wal [Hinv' [Hfr [HreprVal [HrelE' [HvalsPreserved Hstep]]]]]]].
        ...
        split. {
          ...
          assert (Wasm_int.Int64.eq w2 w2 = true) by now apply Wasm_int.Int64.eq_true. now rewrite H21.
          ...
          assert ([:: $VN nat_to_value 1] = [:: $V VAL_num (VAL_int32 (wasm_value_to_i32 wal))]). {
            assert (wal = Val_unboxed 1%N). { inv HreprVal; try discriminate. now replace ord with 0%N by congruence. inv H32. now replace arity with 0 in * by congruence. }
            now rewrite H21. }
          rewrite H21.
          now apply Hstep. }
        try repeat (split; auto). all: subst fr; auto.
      ...
      { (* w1 <> w2 *)
        assert ((i1 =? i2)%uint63 = false). {
          ...
          rewrite Hid1 in H14. rewrite Hid2 in H14.
          unfold not in H14.
          intro Hcontra.
          assert (to_Z i1 = to_Z i2) by congruence. now apply H14 in H18. }
        rewrite H14 in HprimResSome.
        assert (v = Vconstr t_false []) by congruence.
        assert (get_ctor_ord cenv t_false = Ret 1%N). { unfold get_ctor_ord. now rewrite H17. }
        assert (get_ctor_arity cenv t_false = Ret 0). { unfold get_ctor_arity. now rewrite H17. }
        destruct (H4 _ _ H18 H20 H19) as [fr [wal [Hinv' [Hfr [HreprVal [HrelE' [HvalsPreserved Hstep]]]]]]].
        simpl_modulus. cbn. lia.
        exists s, fr.
        split. {
          ...
          assert ([:: $VN nat_to_value 3] = [:: $V VAL_num (VAL_int32 (wasm_value_to_i32 wal))]). {
            assert (wal = Val_unboxed 3%N). { inv HreprVal; try discriminate. now replace ord with 1%N by congruence. now replace arity with 0 in * by congruence. } 
            now rewrite H21. }
          rewrite H21.
          now apply Hstep. }
        try repeat (split; auto). all: subst fr; auto.
