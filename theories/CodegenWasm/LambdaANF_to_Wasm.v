...
Definition apply_binop_and_store_i64 (op : basic_instruction) y1 y2 :=
  [ BI_global_get global_mem_ptr (* to box result of operation *)
    ; BI_local_get y1
  ...
  | "prim_int63_eqb" =>
      Ret (arg_instrs ++ [ BI_relop T_i64 (Relop_i ROI_eq)
                           ; BI_if (BT_valtype (Some (T_num T_i32)))
                               [ BI_const_num (nat_to_value 1) ] (* True *)
                               [ BI_const_num (nat_to_value 3) ] (* False *)
        ])
  ...
  | "prim_int63_compare" =>
      Ret (arg_instrs ++
             [ BI_relop T_i64 (Relop_i ROI_eq)
               ; BI_if (BT_valtype (Some (T_num T_i32)))
                   [ BI_const_num (nat_to_value 1) ] (* Eq *)
                   (arg_instrs ++
                      [ BI_relop T_i64 (Relop_i (ROI_lt SX_U))
                        ; BI_if (BT_valtype (Some (T_num T_i32)))
                            [ BI_const_num (nat_to_value 3) ] (* Lt *)
                            [ BI_const_num (nat_to_value 5) ] (* Gt *)
        ]) ])
...
Definition translate_primitive_operation (nenv : name_env) (lenv : localvar_env) (x_var : localidx) (p : (kername * string * bool * nat)) (args : list var) : error (list basic_instruction) :=
  ...
  (* let arg_instrs := List.flat_map (fun l => [BI_local_get l ; BI_load T_i64 None 2%N 0%N]) arg_vars in *)
               ...
               (* | "prim_int63_diveucl" =>
                   ...
                   | [ x ; y ] =>
                       Ret [ BI_local_get y
                         ...
                         (* Store pair ord *)
                         ; BI_global_get global_mem_ptr
                         ; BI_const_num (nat_to_value 0) (* pair *)
