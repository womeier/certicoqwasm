...
Lemma write_bytes_read_bytes : forall m addr v m0,
  length v = 4 ->
...
(*   intros. do 5 destruct v=>//. cbn in H0.
  ...
  destruct (mem_update (addr + 1 + 1) _ _) eqn:Hu3=>//.
  destruct (mem_update (addr + 1 + 1 + 1) _ _) eqn:Hu4=>//. inv H0.
  unfold read_bytes, those. cbn.
  erewrite mem_update_lookup.
  erewrite mem_update_lookup; eauto.
  erewrite mem_update_lookup.
  erewrite mem_update_lookup; eauto. reflexivity. *)
Admitted.
...
Proof.
  ...
  cbn.
  now erewrite write_bytes_read_bytes.
Qed.
...
Proof.
  ...
  - (* i32 *)
    ...
    2:{ apply memory.mem_grow_length in Hgrow. inv H2. unfold mem_length in Hb', Hb.
...
Lemma load_store_load_i32' : forall m m' a1 a2 v w,
  ...
  load_i32 m' a1 = Some v.
Proof.
  unfold load_i32, load, store, write_bytes_meminst. intros.
  destruct (a1 + (0 + 4) <=? mem_length m)%N eqn:Hlen=>//.
  destruct (a2 + 0 + N.of_nat 8 <=? mem_length m)%N eqn:Hlen'=>//.
  destruct (read_bytes m (a1 + 0) (ssrnat.nat_of_bin 4)) eqn:Hread=>//.
  destruct (write_bytes (meminst_data m) (a2 + 0) (bytes_takefill #00 8 w)) eqn:Hwrite=>//.
  have Hlen'' := write_bytes_preserve_length Hwrite.
  inv H1. inv H2. unfold mem_length. cbn. rewrite -Hlen''.
  unfold mem_length in Hlen.
  destruct (a1 + 4 <=? memory.mem_length (meminst_data m))%N eqn:Hcontra; try lia.
Admitted.
