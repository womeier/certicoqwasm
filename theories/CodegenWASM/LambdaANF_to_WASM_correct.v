...
| Rfunction_v : forall x rho rho' env fds f sr fr tag vs e e' idx ftype ts body,
      repr_funvar f idx ->
...
Proof.
  intros ? ? ? Hv1 Hv2.
  inv Hv1. inv Hv2. unfold translate_function_var in H, H0.
...
Proof.
  ...
  { inv H2. apply IHrepr_expr_LambdaANF_Codegen in H9.
    have H' := repr_var_deterministic _ _ _ _ H11 H1.
...
Definition INV_table_id sr fr :=
  forall f i errMsg,
    translate_function_var nenv fenv f errMsg = Ret i ->
...
Proof.
  ...
  { (* g <> global_mem_ptr *)
    ...
    cbn in Heqn'. rewrite nth_error_update_list_others in Heqn'.
    rewrite Heqn'. reflexivity. intro. subst. apply n.
    eapply NoDup_nth_error; eauto; try congruence.
    now apply nth_error_Some. apply /ssrnat.leP. now apply nth_error_Some. } auto. }
Qed.
...
Lemma update_mem_preserves_table_id : forall s s' f m,
   INV_table_id s f ->
   upd_s_mem (host_function:=host_function) s
                (update_list_at (s_mems s) 0 m) = s' ->
   INV_table_id s' f.
...
Lemma update_mem_preserves_types : forall s s' f m,
   INV_types s f ->
   upd_s_mem (host_function:=host_function) s
                (update_list_at (s_mems s) 0 m) = s' ->
   INV_types s' f.
...
Lemma update_mem_preserves_fvar_idx_inbounds : forall s s' m,
...
Corollary update_mem_preserves_INV : forall lenv s s' f m m',
  INV lenv s f ->
  nth_error (s_mems s) 0  = Some m ->
  (mem_length m' >= mem_length m)%N ->
  mem_max_opt m' = Some max_mem_pages ->
...
Proof.
  ...
  apply /ssrnat.leP. rewrite -length_is_size. lia.
Qed.
...
Proof with eauto.
  ...
  { exists sr. split.
    ...
    { split; auto. intros. unfold max_mem_pages in *.
      ...
      repeat rewrite Wasm_int.Int32.Z_mod_modulus_id in H3; try lia. apply Nat2Z.inj in H3. subst gmp.
  ...
  { inv H. inv H6. rename s' into instr_args. rename a into y.
    ...
    { (* store var *)
      ...
      assert (Hinv_before_IH : INV lenv s_before_IH f). {
       ...
       unfold update_list_at. rewrite take0. reflexivity. assumption.
     ...
     apply mem_length_upper_bound in Hmem5. cbn in Hmem5. simpl_modulus. cbn. lia. }
      ...
      { rewrite H1 in Hgmp. remember (4 + 4 * offset + cap) as n. inv Hgmp.
        ...
        cbn. unfold update_list_at. rewrite take0. reflexivity.
...
Proof.
 ...
 (* INV after set_global gmp *)
  assert (Hinv_before_args : INV lenv s_before_args f). {
    ...
    subst s_tag. cbn. unfold update_list_at. rewrite take0. reflexivity. assumption.
...
Lemma local_var_mapping_list_nth_error_idx : forall l i n x err,
  NoDup l ->
  nth_error l i = Some x ->
  translate_var nenv (create_local_variable_mapping' n l (M.empty _)) x err = Ret (n + i).
Proof.
  induction l; intros.
  - destruct i; inv H0.
...
Theorem repr_bs_LambdaANF_Codegen_related :
  ...
  forall lenv (rho : eval.env) (v : cps.val) (e : exp) (n : nat) (vars : list cps.var) (fds : fundefs),
    map_injective lenv ->
    vars = ((collect_local_variables e) ++ (collect_function_vars (Efun fds e)) ++
            fds_collect_local_variables fds)%list ->
    NoDup vars ->
...
Proof with eauto.
        ...
        { (* local vars *)
          ...
          { (* x <> x1 *)
            ...
            assert (x1' <> x'). { intro. subst x1'.
              ...
              specialize H8 with ""%bs. unfold translate_var in H8. inv H11.
  ...
  - (* Eapp *)
    ...
    { (* indirect call*)
     ...
     elimr_nary_instr 0. apply r_get_local. eassumption. cbn.
     dostep. apply r_eliml. apply map_const_const_list.
     ...
     { (* type *)
       ...
       rewrite Htype. 2: { inv HeRestr. congruence. } rewrite -Hlen.
     ...
     reflexivity. reflexivity. subst; reflexivity. subst; reflexivity.
    ...
    { (* direct call *) inv H3. rewrite map_cat. cbn.
       ...
       (* prepare IH *)
       remember (Build_frame
                  (map nat_to_value args ++
                     (repeat (nat_to_value 0) (length (collect_local_variables e))))
                  (f_inst fr)) as f_before_IH.
       ...
       assert (Hinv_before_IH : INV lenv_before_IH sr f_before_IH). {
          ...
          intros. inv H3. apply local_var_mapping_list in H5.
          rewrite app_length in H5.
          apply const_val_list_length_eq in HfargsRes.
          apply get_list_length_eq in H0. rewrite <- HfargsRes.
          rewrite <- H0. assumption. }

      assert (Hexpr: exists e', repr_expr_LambdaANF_Codegen (lenv:=lenv_before_IH) fenv nenv e e'). { admit. (* from global assumption about fds *) }
      ...
      assert (Hrelm: rel_mem_LambdaANF_Codegen (lenv:=lenv_before_IH) fenv nenv host_function
        ...
        { (* vars *)
          ...
          assert (In x ys). {(*
            destruct (In_dec Pos.eq_dec x ys); auto. exfalso.
            assert (rho' = M.empty _) by admit. subst rho'. *)
           admit. (* because functions are closed *) }
          assert (Hocc: occurs_free (Eapp f t ys) x). { now constructor. }
          ...
          { clear Hlocs. (* args are already on the stack *)
            ...
            exists j. split.
            intro. eapply local_var_mapping_list_nth_error_idx with (n := 0). (* nodup *) admit.
      ...
      assert (HlenvInjective': map_injective lenv_before_IH). {
        ...
        apply create_local_variable_mapping_injective. admit. (* vgl folgendes *) }
      ...
      { (* table identity map *)
       have I := Hinv. destruct I as [_ [_ [_ [_ [_ [_ [_ [_ [_ [_ [_ [Htableid _]]]]]]]]]]]].
       eapply Htableid. eassumption. }

        admit. admit. admit. subst. cbn in Hfinst. split; auto. }
...
Proof.
  ...
  rewrite MCList.skipn_S skipn_O in Hinit.
...
Lemma init_tab_nth_error_other : forall s s' f t n n' val whatever,
  n <> n' ->
  n' < length (table_data t) ->
  inst_tab (f_inst f) = [0] ->
  s_tables s = [t] ->
...
Lemma init_tab_preserves_length : forall s s' f t t' n n' whatever,
  n' < length (table_data t) ->
  inst_tab (f_inst f) = [0] ->
  s_tables s = [t] ->
  init_tab host_function s (f_inst f) n' {| modelem_table := (Mk_tableidx 0)
...
Lemma init_tabs_effect : forall iis vvs t s s' f whatever,
  ...
  inst_tab (f_inst f) = [0] ->
  exists t', (forall i, In i iis ->
                              s_tables s' = [t']
                           /\ nth_error (table_data t') i = Some (Some i)
                           /\ table_max_opt t' = None) /\
             (forall i, ~ In i iis -> s_tables s' = [t']
                          /\ nth_error (table_data t') i = nth_error (table_data t) i
                          /\ table_max_opt t' = None).
Proof.
  ...
  { cbn in Hvvs. subst vvs.
    ...
    have HlenEq := init_tab_preserves_length _ _ _ _ _ _ _ _ Halen HinstT Ht Logic.eq_refl Ht'. clear Halen.
    ...
    have IH := IHiis vvs _ _ _ _ _ HinstF Ht' Heqvvs Hnodup' Hilen' Htnone Hs' HinstT.
    destruct IH as [t'' [IH1 IH2]].
    exists t''. cbn. split; intros.
    { (* if i in (a::is) then is set in table *)
      destruct H.
      { (* i=a *) subst a.
        assert (HnotIn: ~ In i iis) by (now inv Hnodup).
        apply IH2 in HnotIn. destruct HnotIn as [Ht'' [Hteq Htnone']].
        split; auto. rewrite Hteq.
        ...
        assert (tfinal = t') by congruence. subst tfinal. split; auto. }
      { (* i<>a *) now apply IH1. }
    }
    { (* if i not in (a::is) then the previous value is still in table *)
      ...
      destruct (IH2 _ Hnotin) as [IH1' [IH2' IH3']]. split; auto. split; auto.
      rewrite IH2'.
      ...
      have H' := init_tab_nth_error_other _ _ _ _ _ _ _ _ Hia Halen HinstT Ht Logic.eq_refl HeqsEnd. destruct H' as [t''' [Ht1 Ht2]]. congruence.
   }}
Qed.

Lemma init_tabs_effect : forall iis i v s' s f,
  NoDup (combine iis (table_element_mapping (length iis) 0)) ->
  In (i, v) (combine iis (table_element_mapping (length iis) 0)) ->
  s' = init_tabs host_function s (f_inst f) iis (table_element_mapping (length iis) 0) ->
  exists t, (s_tables s') = [t] /\ nth_error (table_data t) i = Some v.

Proof.
  (* intros. unfold init_tabs in H0. *) unfold init_tabs.
  intros.
  induction e_offs; intros. subst. cbn. rewrite H. destruct t; cbn in H0; inv H0.
  eexists. split. reflexivity. intros. destruct s. cbn. esplit. split.  eexists. split. cbn in H. admit. cbn in *.
  have IH := IHe_offs _ _ _ _ Logic.eq_refl. Ht.
  edestruct IH as [l [ Hl Hn]].
  eexists. split. admit. intros.
  eapply IHe_offs in H0.
  destruct idx. cbn. admit.  subst.
  have IH := IHe_offs _ _ _ _ _  Logic.eq_refl.
  have IH' := IH _ _ (idx - 1).
  eexists; split. admit. intros.
            i < Datatypes.length e_offs ->
            nth_error e_offs i =
            Some (nat_to_i32 (i + idx))). { admit. }
  have IH := IHe_offs _ _ _ _ _ _ H2.
  cbn in H0.
  remember (init_tab host_function s' (f_inst f)
          (Z.to_nat (Wasm_int.Int32.intval a))
          {|
            modelem_table := Mk_tableidx 0;
            modelem_offset := [BI_const (nat_to_value idx)];
            modelem_init := [Mk_funcidx idx]
          |}) as s''.
  have IH := IHe_offs _ _ _ _ _ H0.
  assert ((forall i : nat,
      i < Datatypes.length e_offs ->
      nth_error e_offs i = Some (nat_to_i32 (i + S idx)))). {
      intros.
      cbn in H1. assert (i < S (length e_offs)) by lia.
      apply H1 in H3. cbn in H3. specialize H1 with i.
  cbn in H0.
  { (* e_offs = [] *) H1. subst. rewrite H. destruct t.
    unfold init_tabs in H0; cbn in H0. subst. cbn. rewrite -H.
    cbn. eexists. split. auto. intros. inv H0. }
  { (* e_offs = a :: e_offs' *)
     cbn in *. subst.
     eexists. split. admit.
     intros.
  ...
  { destruct e_offs. 1: { cbn in Hlen. lia. }
    cbn. cbn in *.
    remember (init_tab host_function s' (f_inst f)
             (Z.to_nat (Wasm_int.Int32.intval i))
             {|
               modelem_table := Mk_tableidx 0;
               modelem_offset := [BI_const (nat_to_value 0)];
               modelem_init := [Mk_funcidx 0]
             |}) as s''.
    unfold init_tab in Heqs''.
    rewrite Hinst -Ht in Heqs''. cbn in Heqs''.
    rewrite MCList.skipn_S skipn_O in Heqs''.
    replace ((ssrnat.addn_rec
                             (Z.to_nat (Wasm_int.Int32.intval i)) 1)) with
        (S (Z.to_nat (Wasm_int.Int32.intval i))) in Heqs''.
        2: { unfold ssrnat.addn_rec. cbn. lia. }
    rewrite MCList.skipn_S in Heqs''. subst. cbn. }
...
Proof.
  ...
  have H' := init_tabs_effect _ 0 _ _ _ F2.
(* have H' := add_tab_effect
  destruct H' as [HT0 [HT1 [HT2 HT3]]]. cbn in HT0, HT1, HT2, HT3.
...
Proof.
  ...
  { (* top exp is not Efun _ _ *)
    ...
    assert (Hrelm : rel_mem_LambdaANF_Codegen fenv (lenv:=create_local_variable_mapping (collect_local_variables e)) nenv host_function e (M.empty _) sr f_before_IH Fnil). {
      split.
