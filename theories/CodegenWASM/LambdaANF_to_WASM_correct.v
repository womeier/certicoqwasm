...
Theorem subval_or_eq_fun_not_prim :
 forall v p,
  (forall p', v <> Vprim p') ->
...
| Rfunction_v : forall fds f sr fr tag xs e e' idx ftype ts body,
      ...
      repr_val_LambdaANF_Codegen (LambdaANF.cps.Vfun (M.empty _) fds f) sr fr (Val_funidx idx)
...
Proof.
  ...
  { have indPrinciple := repr_val_constr_args_LambdaANF_Codegen_mut fenv nenv host_function
    (fun (v : cps.val) (s : datatypes.store_record host_function) (f : frame) (w : wasm_value)
         ...
         (forall f',
                f_inst f = f_inst f' ->
                repr_val_LambdaANF_Codegen fenv nenv host_function v s f' w)
    )
    (fun (l : seq cps.val) (s : datatypes.store_record host_function) (f : frame) (i : immediate)
                       (H: repr_val_constr_args_LambdaANF_Codegen fenv nenv host_function l s f i) =>
         (forall f',
                f_inst f = f_inst f' ->
...
Lemma val_relation_func_depends_on_funcs : forall val s s' f i,
  s_funcs s = s_funcs s' ->
  repr_val_LambdaANF_Codegen fenv nenv host_function val s f (Val_funidx i) ->
  repr_val_LambdaANF_Codegen fenv nenv host_function val s' f (Val_funidx i).
Proof.
  intros ? ? ? ? ? Hfuncs Hval.
...
Proof.
  ...
  (* constructor value *)
  { have indPrinciple := repr_val_constr_args_LambdaANF_Codegen_mut fenv nenv host_function
    (fun (v : cps.val) (s : datatypes.store_record host_function)
     (f : frame) (w : wasm_value) (H: repr_val_LambdaANF_Codegen fenv nenv host_function v s f w) =>
         (forall a s' f' m m',
                f_inst f = f_inst f' ->
                s_funcs s = s_funcs s' ->
...
Lemma value_bounds : forall wal v sr fr,
  INV_num_functions_upper_bound sr ->
  repr_val_LambdaANF_Codegen fenv nenv host_function v sr fr wal ->
...
Lemma extract_constr_arg : forall n vs v sr fr addr m,
  ...
  repr_val_constr_args_LambdaANF_Codegen fenv nenv host_function vs sr fr addr ->
...
Lemma memory_grow_reduce_need_grow_mem {lenv} : forall grow state s f gmp m,
  ...
  exists s', reduce_trans
   ...
   (state, s', f, [])
   /\ s_funcs s = s_funcs s'
   /\ (forall wal val, repr_val_LambdaANF_Codegen fenv nenv host_function val s f wal ->
                       repr_val_LambdaANF_Codegen fenv nenv host_function val s' f wal)
...
Proof with eauto.
  ...
  have I := Hinv. destruct I as [_ [INVresM_w [_ [INVgmp_w [INVcap_w [INVmuti32 [INVlinmem [HgmpInM [? [? [HglobNodup HinvRest]]]]]]]]]]].
  ...
  { (* growing memory fails *)
    ...
    split. eapply update_glob_keeps_funcs_intact; eassumption.
    split.
    { (* val relation preserved *)
      ...
      erewrite <- update_glob_keeps_memory_intact; eassumption.
      simpl_modulus. cbn. lia.
      eapply global_var_write_read_other; try apply HresM; auto. now intro.
      simpl_modulus. cbn. lia. }
...
Lemma store_constr_args_reduce {lenv} : forall ys offset vs sargs state rho fds s f m v_cap,
  ...
  (forall y y' v, rho ! y = Some v -> repr_funvar fenv nenv y y' ->
         repr_val_LambdaANF_Codegen fenv nenv host_function v s f (Val_funidx y')) ->
  exists s', reduce_trans
            ...
            /\ repr_val_constr_args_LambdaANF_Codegen fenv nenv _ vs s' f (4 + (4*offset) + v_cap)
            /\ sglob_val (host_function:=host_function) s' (f_inst f)
...
Proof.
  ...
  { inv H. inv H6. rename s' into instr_args. rename a into y.
    ...
    assert (Hinstr: exists wal,
      ...
      repr_val_LambdaANF_Codegen fenv nenv host_function v s f wal). {
    ...
    destruct Hinstr as [wal [HinstrRed HinstrVal]].
    {
      ...
      assert (Hmem_before_IH : nth_error (s_mems s_before_IH) 0 = Some m0). {
        subst s'. erewrite <- update_glob_keeps_memory_intact; try eassumption.
      ...
      assert (HrelM_before_IH: (forall y : var,
        ...
        { edestruct i32_exists_nat as [? [Hn ?]]. erewrite Hn in H6.
          ...
          assert (-1 < Z.of_nat (4 + 4 * offset + v_cap) < Wasm_int.Int32.modulus)%Z. {
             ...
             simpl_modulus. apply mem_length_upper_bound in Hmem5; cbn in Hmem5.
             cbn. lia. }
          assert (gmp = (4 + 4* offset + v_cap)). { apply nat_to_i32_eq_modulus; auto. } subst gmp. clear Hgmp.
          ...
          eapply global_var_write_read_same in H6. eassumption.
          simpl_modulus. cbn. lia.
          lia.
     ...
     assert (HfVal_before_IH: (forall (y : positive) (y' : immediate) (v : cps.val),
       ...
       repr_val_LambdaANF_Codegen fenv nenv host_function v s_before_IH f (Val_funidx y'))).
     { intros. have H' := HfVal _ _ _ H7 H8.
      ...
      split. {
        ...
        eapply val_relation_depends_on_mem_smaller_than_gmp_and_funcs; try apply H13;
          try eassumption.
        - reflexivity.
        - apply update_glob_keeps_funcs_intact in H6. subst s'. cbn in H6.
          congruence.
        - apply update_glob_keeps_memory_intact in H6. rewrite <- H6.
          cbn. destruct (s_mems s). inv Hm. subst s'. reflexivity.
        - have I := Hinv. destruct I as [_ [_ [_ [_ [_ [_ [_ [Hgmp_M _]]]]]]]].
...
Lemma store_constr_reduce {lenv} : forall state s f rho fds ys (vs : list cps.val) t sargs,
  ...
  (forall y : var,
         ...
         exists (v6 : cps.val) (val : wasm_value),
           rho ! y = Some v6 /\
           stored_in_locals (lenv:=lenv) nenv y val f /\
           repr_val_LambdaANF_Codegen fenv nenv host_function v6 s f val) ->
  (Z.of_nat (length ys) <= max_constr_args)%Z ->
...
Proof.
 ...
 (* INV after set_global gmp *)
  ...
  assert (HfVal_before_args: (forall (y : positive) (y' : immediate) (v : cps.val),
         rho ! y = Some v ->
         repr_funvar fenv nenv y y' ->
         repr_val_LambdaANF_Codegen fenv nenv host_function v s_before_args f (Val_funidx y'))).
...
Inductive const_val_list : list cps.val -> store_record -> frame -> list nat -> Prop :=
  ...
  | CV_cons : forall s f v vs n w ns,
       repr_val_LambdaANF_Codegen fenv nenv _ v s f w ->
...
Lemma const_val_list_nth_error : forall vs s f ns v j,
  ...
  exists w, repr_val_LambdaANF_Codegen fenv nenv host_function v s f w /\
...
Lemma rel_env_app_letapp {lenv} : forall f t ys rho sr fr fds x e,
  rel_env_LambdaANF_Codegen (lenv:=lenv) fenv nenv host_function (Eletapp x f t ys e) rho sr fr fds ->
  rel_env_LambdaANF_Codegen (lenv:=lenv) fenv nenv host_function (Eapp f t ys) rho sr fr fds.
Proof.
...
(* TODO rename *)
Lemma test_helper : forall rho e v c fds rho' fds' f',
  (forall (x : positive) (rho' : M.t val) (fds' : fundefs) (f' : var) (v0 : val),
	...
	(forall e' e'' eAny fdsAny, (subterm_or_eq e' e \/ (subterm_or_eq e' e'' /\ dsubterm_fds_e e'' fds))
	                             -> e' <> Efun fdsAny eAny) ->
...
Proof.
  ...
  { (* Eapp *)
    ...
    { (* ~In x xs *)
      ...
      now inv Heq.
      apply def_funs_find_def' in H'. destruct H' as [[tuple Hfd] | Hcontra]. 2: inv Hcontra.
      now eapply find_def_name_in_fundefs. destruct H5. now subst.
    }
    intros ? ? ? ? [H' | H']. 2: now eapply HnoFd.
  ...
  { (* Eletapp *)
    ...
    { (* x = x0 *)
      ...
      { (* ~In x0 xs *)
        ...
        now inv Heq.
        apply def_funs_find_def' in H'. destruct H' as [[tuple Hfd] | Hcontra]. 2: inv Hcontra.
        now eapply find_def_name_in_fundefs. destruct H6. now subst.
      }
       intros ? ? ? ? [H' | H']. 2: now eapply HnoFd.
       eapply HnoFd. right. split. eassumption.
       now eapply find_def_dsubterm_fds_e.
...
Lemma repr_expr_LambdaANF_Codegen_no_Efun_subterm {lenv} : forall e_body eAny,
  repr_expr_LambdaANF_Codegen (lenv:=lenv) fenv nenv e_body eAny ->
  ...
  forall lenv (rho : eval.env) (v : cps.val) (e : exp) (n : nat) (vars : list cps.var) (fds : fundefs),
    ...
    forall (hs : host_state) (sr : store_record) (f : frame)
      ...
      exists (sr' : store_record) (f' : frame),
        ...
        (forall wal val, repr_val_LambdaANF_Codegen fenv nenv host_function val sr f wal ->
                         repr_val_LambdaANF_Codegen fenv nenv host_function val sr' f' wal) /\
...
Proof with eauto.
  ...
  - (* Econstr *)
    ...
    { remember (grow_memory_if_necessary page_size) as grow.
     ...
     assert (HfVal' : (forall (y : positive) (y' : immediate) (v : cps.val),
           rho ! y = Some v ->
           repr_funvar fenv nenv y y' ->
           repr_val_LambdaANF_Codegen fenv nenv host_function v s' fr
             (Val_funidx y'))).
  ...
  - (* Eproj ctor_tag t, let x := proj_n y in e *)
    { inv Hrepr_e.
       ...
       {| f_locs := set_nth (wasm_deserialise bs T_i32) (f_locs fr) x' (wasm_deserialise bs T_i32);
        ...
        { (* funs1 *)
          ...
          { (* x <> x1*)
            ...
            have H' := Hfun1 _ _ _ H6 H9. apply Hfun2 with (errMsg:=""%bs) in H'.
        ...
        { (* local vars *)
          ...
          { (* x = x1 *)
            ...
            unfold translate_var in H10. unfold translate_var.
            destruct (lenv ! x); auto. inv H10. cbn.
          ...
          { (* x <> x1 *)
            ...
            assert (x1' <> x'). { intro. subst x1'.
              inv Hx'. unfold translate_var in H13.
              destruct (lenv ! x) eqn:Heqn; inv H13.
              specialize H10 with err_str. unfold translate_var in H10.
              destruct (lenv ! x1) eqn:Heqn'; inv H10.
     ...
     assert (HfenvRho' : forall (a : positive) (v0 : cps.val),
       ...
       apply notNone_Some in H9. apply HfenvWf in H9. destruct H9.
       inv Hx'. destruct HenvsDisjoint as [Hd1 Hd2].
  ...
  - (* Ecase *)
    ...
    assert (HfNone: find_def y fds = None). {
      apply HfenvWf_None with (f:=y) in HfenvWf. rewrite HfenvWf. inv H11.
    ...
    { cbn. rewrite Wasm_int.Int32.Z_mod_modulus_id; lia. }
    cbn. rewrite Harith in H17. unfold load_i32 in H8.
    ...
    { dostep. elimr_nary_instr 1.
     ...
     } unfold load_i32 in H17. rewrite Hload in H17. inv H17. auto. }
  ...
  - (* Eapp *)
    { inv Hrepr_e. rename args' into args_instr.
     ...
     /\ repr_val_LambdaANF_Codegen fenv nenv _ (Vfun (M.empty _) fds f') sr fr (Val_funidx fidx)). {
      inv H8.
      { (* indirect call *)
    ...
    assert (Hrelm: rel_env_LambdaANF_Codegen (lenv:=lenv_before_IH) fenv nenv host_function
      ...
      { (* funs1 *) intros.
       ...
       { (* ~In x xs *)
         ...
         destruct H10 as [? [? [? ?]]].
         apply Hfds with (errMsg:=""%bs) in H3. destruct H3 as [_ [_ [_ [fidx' [HtransF Hval]]]]].
         inv Hval. eapply find_def_name_in_fundefs. eassumption.
      }} split.
      { (* fun2 *)
  ...
  - (* Eletapp *)
    ...
    { inv Hrepr_e. rename args' into args_instr. rename e into e_cont, x into x_res.
      ...
      assert (Hval: exists fidx,
        reduce_trans (state, sr, fr, [AI_basic instr])
                     (state, sr, fr, [AI_basic (BI_const (nat_to_value fidx))])
     /\ repr_val_LambdaANF_Codegen fenv nenv _ (Vfun (M.empty _) fds f') sr fr (Val_funidx fidx)
     /\ exists e_body', repr_expr_LambdaANF_Codegen fenv nenv e_body e_body'
    ...
    assert (Hrelm: rel_env_LambdaANF_Codegen (lenv:=lenv_before_IH) fenv nenv host_function
      ...
      { (* funs1 *) intros.
       ...
       { (* ~In x xs *)
         ...
         destruct H'. 2: inv H5.
         apply subval_fun in H3. 2:{ destruct H5. now eapply find_def_name_in_fundefs. }
         destruct H3 as [f1 [?H ?H]]. inv H3. assumption. }
      } split.
      { (* funs2 *)
    ...
    have IH_body := IHHev1 Hnodup' HfenvRho' HeRestr' Hunbound' _  HlenvInjective' HenvsDisjoint'
                   state _ _ _ HfdsEqRhoEmpty_before_IH Hfds_before_IH Hinv_before_IH Hexpr Hrelm.
    clear HfenvRho' Hnodup' Hunbound' HeRestr'.

    destruct IH_body as [sr_after_call [fr_after_call [Hred [Hval [Hfinst [Hfuncs [HvalPres H_INV]]]]]]]. clear IHHev1.
    ...
    assert (Hcont: exists (sr_final : store_record) (fr_final : frame),
      reduce_trans (state, sr_after_call, fr, [ AI_basic (BI_get_global result_out_of_mem)
         ...
         /\ sr_final.(s_funcs) = sr.(s_funcs)
            (* previous values are preserved *)
         /\ (forall wal val, repr_val_LambdaANF_Codegen fenv nenv host_function val sr fr wal ->
                             repr_val_LambdaANF_Codegen fenv nenv host_function val sr_final fr_final wal)
      ...
      { (* success *)
        ...
        assert (HfdsEqRhoEmpty_before_cont_IH: (forall (x : positive) (rho' : M.t val)
          ...
          { (* x = x_res *)
            ...
            have H' := test_helper _ _ _ _ fds _ _ _ _ _ Hev1 Hval'.
            edestruct H'.
            { intros ? ? ? ? ? Hrho Hsubval.
              assert (Hdec: decidable_eq var). { intros n m.
              unfold Decidable.decidable. now destruct (var_dec n m). }
            ...
            { intros ? ? ? ? [HbodyNofun | HfdsNofun].
              ...
              - destruct HfdsNofun as [Hsub HsubFds].
                ...
                2:{ now apply NoDup_app_r in Hnodup. }
        ...
        assert (HrelM': rel_env_LambdaANF_Codegen (lenv:=lenv) fenv nenv host_function e_cont
                   (map_util.M.set x_res v rho) sr_after_call f_before_cont fds). {
          ...
          { (* funs1 *)
            ...
            { (* x = x_res *)
              ...
              have H' := test_helper _ _ _ _ fds _ _ _ _ _ Hev1 H3.
              edestruct H'.
              { intros ? ? ? ? ? Hrho Hsubval'.
                assert (Hdec: decidable_eq var). { intros n m.
                unfold Decidable.decidable. now destruct (var_dec n m). }
        ...
        destruct IH_cont as [sr_final [fr_final [Hred' [Hval' [Hfinst' [Hfuncs' [HvalPres' H_INV']]]]]]].
    ...
    destruct Hcont as [sr_final [fr_final [Hred_final [Hval_final [Hfinst' [Hfuncs' [HvalPres' H_INV']]]]]]]. clear IHHev2.
    ...
    { (* type *)
      ...
      rewrite Htype. 2: { inv HeRestr. congruence. } rewrite -Hlen. cbn. inv H9.
      now rewrite map_repeat_eq. } apply rt_refl.
   rewrite catA. cbn. eapply rt_trans. eapply app_trans with (es := ([seq AI_basic (BI_const (nat_to_value a)) | a <- args] ++
    [:: AI_invoke fidx])).
    (* enter function *)
    dostep'. eapply r_invoke_native with (vcs:= map (fun a => (nat_to_value a)) args)
                                        (f':=f_before_IH); try eassumption.
  ...
  - (* Ehalt *)
    ...
    simpl_modulus. cbn. lia.
    eapply global_var_write_read_other; try eassumption. now intro. split; first auto.
    split. eapply update_glob_keeps_funcs_intact; eassumption.
    split. { intros.
      assert (nth_error (s_mems s') 0 = Some m). {
        now rewrite -(update_glob_keeps_memory_intact _ _ _ _ _ Hs). }
      assert (sglob_val (host_function:=host_function) s' (f_inst fr) global_mem_ptr =
                                                    Some (VAL_int32 (nat_to_i32 x'))). {
      erewrite global_var_write_read_other; try eassumption. reflexivity. now intro.
    }
...
Lemma translate_funcs_find_def : forall fds f fns t ys e fenv,
  ...
  exists idx e' locs ftype func, repr_funvar fenv nenv f idx /\
    ...
    repr_expr_LambdaANF_Codegen fenv nenv e e'
...
Proof.
  ...
  { (* f0=v *)
    ...
    apply translate_exp_correct in Hexp; eauto. eapply HcorrCenv with (f:=v). cbn.
