...
Lemma val_relation_depends_only_on_finst : forall v sr f f' wal,
  f_inst f = f_inst f' ->
  repr_val_LambdaANF_Codegen fenv venv nenv host_function v sr f wal ->
  repr_val_LambdaANF_Codegen fenv venv nenv host_function v sr f' wal.
Proof.
  intros ? ? ? ? ? Hfinst Hval.
  have indPrinciple := repr_val_LambdaANF_Codegen_mut fenv venv nenv _
    ...
    (fun (l : seq cps.val) (s : datatypes.store_record host_function) (f : frame) (i : immediate)
      (H: repr_val_constr_args_LambdaANF_Codegen fenv venv nenv host_function l s f i) =>
        forall f' l, f_inst f = f_inst f' ->
                   repr_val_constr_args_LambdaANF_Codegen fenv venv nenv host_function l s f i ->
                   repr_val_constr_args_LambdaANF_Codegen fenv venv nenv host_function l s f' i).
  eapply indPrinciple in Hval; clear indPrinciple; intros; try eauto.
  { inv H1. eapply Rconstr_v; eauto. congruence. }
  { inv H0. eapply Rfunction_v; eauto. }
Admitted.


Lemma val_relation_depends_only_on_mem_and_funcs : forall v sr sr' m m' fr fr' gmp gmp' value,
    f_inst fr = f_inst fr' ->
...
Proof.
  ...
  { remember (4 + addr) as a. generalize dependent addr.
    ...
    Check repr_val_constr_args_LambdaANF_Codegen_mut.
    induction H16 using repr_val_constr_args_LambdaANF_Codegen_mut with
      (P := (fun (v : cps.val) (s : datatypes.store_record host_function) (f : frame) (w : wasm_value)
           (H : repr_val_LambdaANF_Codegen fenv venv nenv host_function v s f w) => forall s' f' m m',
                f_inst f = f_inst f' ->
                s_funcs s = s_funcs s' ->
                ...
                gmp' >= gmp ->
                (forall a, (a + 4<= N.of_nat gmp)%N -> load_i32 m a = load_i32 m' a) ->
                    repr_val_LambdaANF_Codegen fenv venv nenv host_function v s' f' w)); intros.
                    Admitted.
  (*  { rename H0 into m'. rename H into s. econstructor; eauto. lia. admit. rewrite -H15.
    lia. }

      (P0:= (fun (l : seq cps.val) (s : datatypes.store_record host_function) (f : frame) (i : immediate)
           (H: repr_val_constr_args_LambdaANF_Codegen fenv venv nenv host_function l s f i) => forall H1 H2, True)). forall s' f' m m',
                f_inst f = f_inst f' ->
                s_funcs s = s_funcs s' ->
                nth_error s.(s_mems) 0 = Some m ->
                nth_error s'.(s_mems) 0 = Some m' ->
                ...
                (forall a, (a + 4 <= N.of_nat gmp)%N -> load_i32 m a = load_i32 m' a) ->
                   repr_val_constr_args_LambdaANF_Codegen fenv venv nenv host_function l s' f' i)).
    eapply indPrinciple in H16; intros; clear indPrinciple; subst; try eassumption.
    { econstructor; try eassumption; try reflexivity; try lia. rewrite -H8. assumption. inv H16.
      assert (gmp0 = gmp'). admit. subst gmp0. lia. lia. }
    { assert (m = m0) by congruence. subst m0.
      assert (gmp0 = gmp). { admit. } subst gmp0. econstructor; try eassumption; try lia. reflexivity.
        rewrite -H17. assumption. lia. eapply H1; eauto. }
    { econstructor; eauto. erewrite <-e1. congruence. }
    { assert (gmp0 = gmp'). admit. subst gmp0. econstructor; eauto; lia. }
    { assert (m = m0) by congruence. subst m0. econstructor; eauto. rewrite -H18; auto. admit. }
    { admit. }
    { admit. }
    { inv H16. assert (gmp0 = gmp). admit. subst gmp0. lia. admit. }
    { admit. }
    { reflexivity. }
  }
  (* function *)
  ...
  { inv H. inv H6. rename s' into instr_args. rename a into y.
    ...
    { (* store var *)
      ...
      assert (exists m0, store m
       ...
       assert (Z.of_nat n + 4 < Z.of_N page_size)%Z. { cbn in H2.
      ...
      assert (Hcap_before_IH: sglob_val (host_function:=host_function) s_before_IH
       (f_inst f) constr_alloc_ptr = Some (VAL_int32 (nat_to_i32 v_cap))). { subst. rewrite -Hcap. reflexivity. }
      ...
      exists (v6 : cps.val) (val : wasm_value),
        ...
        subst s_before_IH. exists val', wal'. repeat split; try assumption.
      ...
      destruct IH as [sr [fr [Hred [Hinv' [v_cap' [Hv1 [? [Hv2 [? [m1 [Hm2 [? ?]]]]]]]]]]]].
      ...
      { assert (exists v, load_i32 m (N.of_nat a) = Some v). {
          ...
          destruct ((Wasm_int.N_of_uint i32m
           (Wasm_int.Int32.iadd (nat_to_i32 v_cap)
              (nat_to_i32 (S (S (S (S (offset * 4))))))) + 0 +
         N.of_nat (t_length T_i32) <=? mem_length m)%N) eqn:Har. 2: inv Hm0.
         admit. } destruct H9.
        have Ht := load_store_load _ _ _ _ _ _ _ _ H9 Hm0. admit. } }
    { (* store fn index *)
      (* invariants *)
      admit.
    }
...
Proof.
  ...
  assert (INV s' f) as Hinv'. { eapply update_global_preserves_INV. 3: eassumption. assumption. unfold result_out_of_mem, constr_alloc_ptr. lia. }
  have I' := Hinv'. destruct I' as [_ [_ [_ [INVgmp_w' [INVcap_w' [INVmuti32' [_ [_ [INVgmp_M' _]]]]]]]]].
  have INVgmp_r' := global_var_w_implies_global_var_r _ _ _ INVmuti32' INVgmp_w'.
  destruct INVgmp_r' as [gmp_v' ?].
  assert ((nat_to_i32 gmp_v) = gmp_v'). {
    assert (Huneq: global_mem_ptr <> constr_alloc_ptr). { unfold global_mem_ptr, constr_alloc_ptr. lia. }
    assert (Htmp: sglob_val (host_function:=host_function) s
       (f_inst f) global_mem_ptr = Some (VAL_int32 gmp_v')). {
     destruct Hinv as [_ [_ [_ [Hgmp_r [_ [Hmuti32 _]]]]]].
 ...
 (* invariants after set_global gmp *)
  ...
  have INVcap_r'' := global_var_w_implies_global_var_r _ _ _ INVmuti32'' INVcap_w''.
   destruct INVcap_r'' as [cap_v Hg2]. clear INVmuti32''.
  (* invariants mem *)
  ...
  destruct Hinv' as [_ [_ [_ [_ [_ [_ [Hlinmem _]]]]]]].
    destruct Hlinmem as [Hmem1 [m [Hmem2 [size [Hmem3 [Hmem4 Hmem5]]]]]].
  have Htest := INVgmp_M' _ _ Hmem2 H1. assert (m'' = m). { apply update_global_var_preserves_memory in H2. congruence. }
  subst m''. lia. }
  ...
  assert (Hmem: nth_error (s_mems s_tag) 0 = Some m'''). { subst s_tag. cbn. unfold update_list_at.
   now rewrite take0. }
  assert (Hglob_cap: sglob_val (host_function:=host_function) s_tag
    ...
    eapply (global_var_write_read_other _ global_mem_ptr); eauto. unfold constr_alloc_ptr, global_mem_ptr. lia. }
...
exists (v6 : cps.val) (val : wasm_value),
  rho ! y = Some v6 /\
  stored_in_locals venv nenv y val f /\
  repr_val_LambdaANF_Codegen fenv venv nenv host_function v6 s_tag f val). {
    intros y Hy. apply HmemR in Hy. destruct Hy as [val [wal [Hrho' [Hylocal Hval]]]].
    exists val, wal. repeat (split; auto). (* repr_val s -> repr_val s_tag *) admit.
  }
  ...
  destruct Hargs as [s_final [f_final [Hred [Hinv_final [v_cap [Hcap_v [? [Hargs_val [Hglobsame [m' [Hm' [Hmemlen Hmemsame]]]]]]]]]]]].
  {
  cbn in Hargs_val. clear Hsetargs Hrho HenoughM' HlenBound.
  assert (gmp_v = v_cap). { unfold sglob_val, sglob, sglob_ind in *. rewrite Hglobsame in Hglob_cap.
  inv Hglob_cap. cbn in H5. (* rewrite Wasm_int.Int32.Z_mod_modulus_id in H5; try lia. rewrite Wasm_int.Int32.Z_mod_modulus_id in H5; lia. *) admit. } subst v_cap.
  ...
  subst s_tag. cbn. apply Hred. }
  { rewrite -Hglob_cap. subst s_tag. reflexivity. }
  split. assumption.
  ...
  unfold nat_to_i32. cbn. rewrite Wasm_int.Int32.Z_mod_modulus_id; lia. } rewrite Heq in Hstore'''.
...
Proof with eauto.
  ...
  - (* Econstr *)
    ...
    { remember (grow_memory_if_necessary page_size) as grow.
      ...
      have Hconstr := store_constr_reduce state _ _ _ _ _ _ _ Hinv' _  HrelM Hmaxargs H9 H.
      edestruct Hconstr as [s_v [f_v [Hred_v [Hinv_v [cap_v [wal [? [? Hvalue]]]]]]]].
      ...
      remember ({|f_locs := set_nth (VAL_int32 (wasm_value_to_i32 wal)) (f_locs f_v) x' (VAL_int32 (wasm_value_to_i32 wal));
      f_inst := f_inst f_v|}) as f_before_IH.
      ...
      separate_instr.
      assert (Heq: [seq AI_basic i | i <- sinit] ++
    [:: AI_basic (BI_get_global constr_alloc_ptr)] ++
    [:: AI_basic (BI_const (nat_to_value (Pos.to_nat t)))] ++
    [:: AI_basic (BI_store T_i32 None 2%N 0%N)] ++
    [seq AI_basic i | i <- sargs] ++ [seq AI_basic i | i <- sres] ++ [seq AI_basic i | i <- e']  =
    ([seq AI_basic i | i <- sinit] ++
    [:: AI_basic (BI_get_global constr_alloc_ptr)] ++
    [:: AI_basic (BI_const (nat_to_value (Pos.to_nat t)))] ++
    [:: AI_basic (BI_store T_i32 None 2%N 0%N)] ++
    [seq AI_basic i | i <- sargs]) ++ [seq AI_basic i | i <- sres] ++ [seq AI_basic i | i <- e']). { rewrite <- app_assoc. reflexivity. }
    cbn. rewrite Heq.
    clear Heq. replace ([::]) with ([::] ++ [::]: list administrative_instruction) by reflexivity.
    eapply rt_trans. apply app_trans. subst. apply Hred_v. clear Hred_v. subst sres. cbn.
    separate_instr.
    ...
    assert (f_inst f_before_IH = f_inst f_v) as Hfinst. { subst. reflexivity. }
