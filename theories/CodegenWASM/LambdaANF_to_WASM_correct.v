...
Lemma store_constr_reduce {lenv} : forall state s f rho ys (vs : list cps.val) t sargs,
  ...
  exists s', reduce_trans
    ...
    (state, s', f, []) /\ INV lenv s' f /\ s_funcs s = s_funcs s'
      (* cap points to value *)
...
exists (v6 : cps.val) (val : wasm_value),
  ...
  split. { apply update_glob_keeps_funcs_intact in H0, H1. subst. cbn in H1. congruence. }
...
Proof with eauto.
  ...
  - (* Econstr *)
    ...
    { remember (grow_memory_if_necessary page_size) as grow.
      ...
      destruct Hgrowmem as [s' [Hred [[Hinv' [Hfuncs [Henoughmem HpreservedVals]]] | HoutofM]]].
      ...
      edestruct Hconstr as [s_v [Hred_v [Hinv_v [Hfuncs' [cap_v [wal [? [? Hvalue]]]]]]]].
         ...
         { (* local vars *)
           ...
           { (* x <> x1 *)
             ...
             eapply val_relation_depends_on_mem_smaller_than_gmp_and_funcs;
               try apply Hval; eauto; try congruence; admit.
             }
        }}
     ...
     assert (Hfds': (forall (x1 : positive) (rho' : M.t cps.val) (fds' : fundefs) (f' : var),
      (map_util.M.set x v rho) ! x1 = Some (Vfun rho' fds' f') -> fds' = fds /\ rho' = M.empty _)). {
        ...
        { subst x1. rewrite M.gss in Hfds'. inv Hfds'. inv Hbsval.
          destruct (Hfds _ _ _ _ H11). subst. split; auto. admit.
          (* nthN true, maybe a little tricky *) }
        (* x<>x1 *)
        { rewrite M.gso in Hfds'; auto. now apply Hfds in Hfds'. }
     }
  ...
  - (* Eapp *)
    ...
    { (* direct call *) inv H3. rewrite map_cat. cbn.
       ...
       assert (Hfds_before_IH: (forall (x : positive) (rho' : M.t cps.val)
         ...
         destruct (Hfds _ _ _ _ H). subst fl rho'.
         intros. admit. (* wahr ggf tricky *) }
...
Proof.
  ...
  assert (Hfds_before_IH: (forall (x : positive) (rho' : M.t val)
           (fds' : fundefs) (f' : var),
         (M.empty _) ! x = Some (Vfun rho' fds' f') -> fds' = Fnil /\ rho' = M.empty _)). {
   intros. inv H. }
