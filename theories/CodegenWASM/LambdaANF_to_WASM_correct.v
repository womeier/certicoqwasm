...
with repr_val_constr_args_LambdaANF_Codegen : (list LambdaANF.cps.val) -> store_record -> immediate -> Prop :=
     ...
     | Rcons_l: forall v vptr vs sr m addr,
        (* store_record contains memory *)
        List.nth_error sr.(s_mems) 0 = Some m ->

        (* constr arg is ptr related to value v *)
        load_i32 m (N.of_nat addr) = Some (VAL_int32 (immediate_to_i32 vptr)) ->
        repr_val_LambdaANF_Codegen v sr (Val_ptr vptr) ->
...
Proof.
  ...
  (* follows from assumption *) admit.
   ...
   have IH := IHHev e' (Build_frame (set_nth (wasm_deserialise bs T_i32) (f_locs fr) x' (wasm_deserialise bs T_i32)) fr.(f_inst)) state sr INVres INVlinmem INVptrInMem INVlocals H7 Hrm. destruct IH as [sr' [f' [Hred Hval]]].
    ...
    assert (Harith: (N.of_nat (4 + addr) + 4 * n)%N = (Wasm_int.N_of_uint i32m
     (Wasm_int.Int32.iadd (wasm_value_to_i32 (Val_ptr addr))
        (nat_to_i32 (N.to_nat n + 1 + (N.to_nat n + 1 + (N.to_nat n + 1 + (N.to_nat n + 1 + 0)))))))). {
...
Proof.
  ...
  inv H. apply IHl in H4. simpl; auto. rewrite H4. reflexivity.
Qed.
...
(* keep around the fact that t is no bigger than 2^(int_size-1) [which we know by correct_crep] *)
 ...
 Proof.
   ...
   -  simpl. destruct p0.
      ...
      intro. inv H0.
      reflexivity.
   - assert (Hp0 := Zlt_neg_0 p0). exfalso. lia.
 Qed.
...
Inductive _LambdaANF_Codegen:  LambdaANF.cps.val -> mem -> Values.val -> Prop :=
| Rint_v: forall  z r m,
    repr_unboxed_Codegen (Z.to_N z) r ->
    _LambdaANF_Codegen (LambdaANF.cps.Vint z) m (make_vint r)
| Rconstr_unboxed_v:
    forall t arr n m,
      M.get t rep_env = Some (enum arr) ->
      repr_unboxed_Codegen arr n ->
      _LambdaANF_Codegen (LambdaANF.cps.Vconstr t nil) m  (make_vint n)
| Rconstr_boxed_v: forall  t vs n a b i m h,
    (* t is a boxed constructor, n ends with 0 and represents
      a pointer to _ptr of (t, vs)  *)
    M.get t rep_env = Some (boxed n a) ->
...
Proof.
  ...
  apply repr_val_id_L_LambdaANF_Codegen_vint_or_vptr in H.
 Admitted.
  ...
  -   assert (length (a :: l) = length vs) by (eapply Forall2_length'; eauto). rewrite H2 in *. clear H2.
     ...
     + (* last statement *)
       ...
       * inv H4.
         ...
         econstructor. apply Hxlenv. Admitted.
...
Proof.
  ...
  {   (* unfold Int64.ltu. rewrite Coqlib.zlt_true. *)
      ...
      rewrite Int64.unsigned_repr by (archi_red; solve_uint_range; lia).
      unfold Int64.iwordsize. unfold Int64.zwordsize. simpl.
      unfold Int64.ltu.
      rewrite Int64.unsigned_repr by (unfold Int64.max_unsigned; solve_uint_range; lia).
      rewrite Int64.unsigned_repr by (unfold Int64.max_unsigned; solve_uint_range; lia).
  unfold classify_shift. simpl. reflexivity.
}
{
  rewrite Int.shru_div_two_p.
  rewrite Int.Zshiftr_div_two_p by lia.
  rewrite Int.unsigned_repr by (archi_red; solve_uint_range; lia).
  unfold Int.iwordsize. unfold Int.zwordsize. simpl.
  unfold Int.ltu.
  rewrite Int.unsigned_repr by (solve_uint_range; lia).
  rewrite Int.unsigned_repr by (solve_uint_range; lia).
  unfold classify_shift. simpl. reflexivity.
}
Qed.
...
Proof.
  ...
  - eapply Forall2_monotonic_strong; eauto.
    ...
    + left.
      ...
      chunk_red; ulia.
      split. apply OrdersEx.Z_as_OT.add_nonneg_nonneg. apply Ptrofs.unsigned_range. chunk_red; ulia.
      eapply OrdersEx.Z_as_OT.le_lt_trans; eauto. chunk_red; ulia.
      split. apply OrdersEx.Z_as_OT.add_nonneg_nonneg. apply Ptrofs.unsigned_range. chunk_red; ulia.
      eapply OrdersEx.Z_as_OT.le_lt_trans; eauto. chunk_red; ulia.
    + right.
      rewrite Ptrofs.add_unsigned.
       ...
       apply Zlt_le_succ.        apply N2Z.inj_lt. auto.
       replace (int_size*Z.of_N i + int_size)%Z with (int_size * (Z.of_N i + 1))%Z  by (chunk_red; ulia).
       chunk_red; ulia.
       split. apply OrdersEx.Z_as_OT.add_nonneg_nonneg. apply Ptrofs.unsigned_range. chunk_red; ulia.
       eapply OrdersEx.Z_as_OT.le_lt_trans; eauto. chunk_red; ulia.
       split. apply OrdersEx.Z_as_OT.add_nonneg_nonneg. apply Ptrofs.unsigned_range. chunk_red; ulia.
...
Proof.
  ...
  -  (* ys = a::ys0 inf = i::inf0 *)
    ...
    apply OrdersEx.Z_as_OT.add_nonneg_nonneg. apply Ptrofs.unsigned_range. chunk_red; ulia.
    auto.
      ...
      - (* boxed *)
       ...
       (Maps.PTree.set allocIdent
        ...
        assert (H_m2_m3 :  mem_after_n_proj_store alloc_b
          ...
          assert (0 <= Z.of_nat (length vs7))%Z by apply Zle_0_nat. chunk_red; lia.
