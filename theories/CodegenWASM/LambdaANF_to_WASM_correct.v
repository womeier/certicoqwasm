...
Inductive repr_val_LambdaANF_Codegen:  LambdaANF.cps.val -> (* val *)
                                        store_record ->     (* sr *)
                                         frame ->           (* fr *)
                                          wasm_value ->     (* wal *)
                                          Prop :=
| Rconstr_unboxed_v : forall v t (sr : store_record) fr uv,
    v = (tag_to_i32 t) ->
    repr_val_LambdaANF_Codegen (LambdaANF.cps.Vconstr t []) sr fr (Val_ptr uv)
...
Proof with eauto.
  ...
  - (* Eprim_val *) admit. (* inv Hrepr_e. *) (* absurd, primitives not supported *)
...
Proof.
  ...
  { (* top exp is Efun _ _ *)
    ...
    2:{ admit. (* eapply Forall_constructors_subterm. eassumption. constructor. *)
        (* apply dsubterm_fds2. *) }
    ...
    assert (Hfds : forall (a : var) (t : fun_tag) (ys : seq var) (e0 : exp) errMsg,
      ...
      { assert (Hc: find_def a fds <> None) by congruence.
        ...
        eapply val_relation_depends_on_finst; last apply Hval.
        subst f_before_IH. admit. admit. admit. admit. (* reflexivity. *) }
    }
