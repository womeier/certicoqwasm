...
| Rconstr_unboxed_v : forall v t (sr : store_record) fr,
    Pos.to_nat (t * 2 + 1) = v ->
    repr_val_LambdaANF_Codegen (LambdaANF.cps.Vconstr t []) sr fr (Val_unboxed v)


| Rconstr_boxed_v : forall v t vs (sr : store_record) fr gmp m (addr : nat),
...
(* for fn values returned by the body of Eletapp, it holds, that rho=M.empty etc. *)
  ...
  [seq AI_basic i | i <- sres] ++ [seq AI_basic i | i <- e']). { rewrite <- app_assoc. reflexivity. }
    ...
    { (* Nullary constructor case *)
      ...
      assert (HfenvRho' : (forall (a : positive) (v : val),
        ...
        intros. apply HfenvRho. rewrite M.gso in H0. assumption.
        intro. subst a. apply notNone_Some in H1. apply HfenvWf in H1. destruct H1. inv H7. destruct HenvsDisjoint as [Hd1 Hd2]. apply Hd2 in H0. unfold translate_var in H1. rewrite H0 in H1. inv H1. assumption.
      }
      ...
      assert (Hunbound' : (forall x0 : var,
        ...
        intros. apply NoDup_app_remove_r in Hnodup. cbn in Hnodup. apply NoDup_cons_iff in Hnodup. rewrite M.gso. apply Hunbound. unfold collect_local_variables. cbn. fold collect_local_variables. right. assumption. destruct Hnodup as [Hx _ ]. unfold not. unfold not in Hx. intros Heq. subst. apply Hx in H0. contradiction.
      }
...
destruct Hinv as [_ [_ [_ [_ [_ [_ [_ [_ [_ [_ [_ [_ [H' _]]]]]]]]]]]]]. destruct (H' x x'). assumption. auto. auto.
  ...
  - (* Eproj ctor_tag t, let x := proj_n y in e *)
    { inv Hrepr_e.
      ...
      have Hextr := extract_constr_arg n vs v _ _ _ _ HfnsUpperBound H0 H6 H13.
      ...
      assert (Hrm: rel_env_LambdaANF_Codegen (lenv:=lenv) fenv nenv host_function e (map_util.M.set x v rho) sr
       {| f_locs := set_nth (wasm_deserialise bs T_i32) (f_locs fr) x' (wasm_deserialise bs T_i32);
        ...
        { (* funs1 *)
          destruct (var_dec x x1).
          { (* x = x1 *)
            subst x1. rewrite M.gss in H8. inv H8. rename v0 into v.
            apply nthN_In in H0.
            have H' := subval_or_eq_constr _ _ _ t H10 H0.
          ...
          { (* x <> x1*)
            rewrite M.gso in H8; auto.
            have H' := Hfun1 _ _ _ H8 H10. apply Hfun2 with (errMsg:=""%bs) in H'.
        ...
        { (* local vars *)
          ...
          { (* x = x1 *)
            ...
            unfold translate_var in H11. unfold translate_var.
            destruct (lenv ! x); auto. inv H11. cbn.
          ...
          { (* x <> x1 *)
            ...
            assert (x1' <> x'). { intro. subst x1'.
              inv Hx'. unfold translate_var in H14.
              destruct (lenv ! x) eqn:Heqn; inv H14.
              specialize H11 with err_str. unfold translate_var in H11.
              destruct (lenv ! x1) eqn:Heqn'; inv H11.
     ...
     assert (HfenvRho' : forall (a : positive) (v0 : cps.val),
       ...
       rewrite M.gso in H8; auto. intro Hcontra. subst a.
       apply notNone_Some in H10. apply HfenvWf in H10. destruct H10.
       inv Hx'. destruct HenvsDisjoint as [Hd1 Hd2].
       apply Hd2 in H8. unfold translate_var in H10. now rewrite H8 in H10. }
     ...
     { (* take steps *)
       ...
       rewrite H in H8. injection H8 => H8'. subst. clear H8.
