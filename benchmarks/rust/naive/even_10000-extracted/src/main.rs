...
fn CertiCoq_Benchmarks_tests_even(&'a self) -> &'a dyn Fn(&'a Coq_Init_Datatypes_nat<'a>) -> bool {
...
fn Coq_Init_Nat_mul(&'a self, n: &'a Coq_Init_Datatypes_nat<'a>, m: &'a Coq_Init_Datatypes_nat<'a>) -> &'a Coq_Init_Datatypes_nat<'a> {
  match n {
    &Coq_Init_Datatypes_nat::O(_) => {
      self.alloc(
...
fn Coq_Init_Nat_mul__curried(&'a self) -> &'a dyn Fn(&'a Coq_Init_Datatypes_nat<'a>) -> &'a dyn Fn(&'a Coq_Init_Datatypes_nat<'a>) -> &'a Coq_Init_Datatypes_nat<'a> {
  self.closure(move |n| {
    self.closure(move |m| {
      self.Coq_Init_Nat_mul(
        n,
