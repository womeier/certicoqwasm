...
fn CertiCoq_Benchmarks_tests_ack(&'a self, n: u64, m: u64) -> u64 {
  __nat_elim!(
    {
      __nat_succ(
        m)
...
fn CertiCoq_Benchmarks_tests_ack__curried(&'a self) -> &'a dyn Fn(u64) -> &'a dyn Fn(u64) -> u64 {
  self.closure(move |n| {
    self.closure(move |m| {
      self.CertiCoq_Benchmarks_tests_ack(
        n,
...
fn CertiCoq_Benchmarks_tests_ack_3_9(&'a self) -> u64 {
  self.CertiCoq_Benchmarks_tests_ack(
    __nat_succ(
      __nat_succ(
        __nat_succ(
