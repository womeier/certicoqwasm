...
fn CertiCoq_Benchmarks_tests_repeat2<A: Copy>(&'a self, A: (), x: A, y: A, n: &'a Coq_Init_Datatypes_nat<'a>) -> &'a Coq_Init_Datatypes_list<'a, A> {
  match n {
...
fn CertiCoq_Benchmarks_tests_repeat2__curried<A: Copy>(&'a self) -> &'a dyn Fn(()) -> &'a dyn Fn(A) -> &'a dyn Fn(A) -> &'a dyn Fn(&'a Coq_Init_Datatypes_nat<'a>) -> &'a Coq_Init_Datatypes_list<'a, A> {
  self.closure(move |A| {
    self.closure(move |x| {
      self.closure(move |y| {
        self.closure(move |n| {
...
pub fn main() {
  Program::new().CertiCoq_Benchmarks_tests_demo2();
}
